        ...
        {card.type === 'scenarioCard' && (() => {
          ...
          function scenarioColor(heading: string, idx: number) {
            ...
            if (h.includes('fix') || h.includes('solution') || h.includes('resolved'))
              return { border: 'border-emerald-500/35', borderOpen: 'border-emerald-500/50', bg: 'bg-emerald-500/10', text: 'text-emerald-700', bar: 'bg-emerald-400' };
            if (h.includes('takeaway') || h.includes('key') || h.includes('lesson') || idx === allItems.length - 1)
              return { border: 'border-amber-500/35', borderOpen: 'border-amber-500/50', bg: 'bg-amber-500/10', text: 'text-amber-700', bar: 'bg-amber-400' };
            return { border: 'border-sky-500/30', borderOpen: 'border-sky-500/45', bg: 'bg-sky-500/8', text: 'text-sky-700', bar: 'bg-sky-400' };
          }
      ...
      <div className="relative w-full overflow-hidden" style={{ height: 720 }}>
        {hasPrev && (
          <div
            ...
            style={{ left: '50%', transform: `translateX(calc(-50% - ${OFFSET}px)) scale(0.88)`, opacity: 0.32 }}
            onClick={prev}
          >
        ...
        <div
          ...
          style={{ left: '50%', transform: 'translateX(-50%) scale(1)', opacity: 1, zIndex: 10 }}
        >
...
export default function MobileShortDetailPage({ params }: { params: { id: string } }) {
  const [short, setShort] = useState<MobileShort | null>(null);
  ...
  async function publish() {
    ...
    try {
      const res = await fetch(`/api/mobile-short/${params.id}/publish`, { method: 'POST' });
      const json = await res.json();
  ...
  if (!short) return <div className="card p-6 text-sm text-red-400">Short not found.</div>;

  const cards = short.cards || [];
    ...
    <div className="flex gap-0 -mx-6 -my-10 min-h-screen">
      ...
      <aside className="w-64 shrink-0 border-r border-[var(--border)] bg-[var(--card)] flex flex-col sticky top-16 self-start max-h-[calc(100vh-4rem)] overflow-hidden">
        <div className="px-4 pt-4 pb-3 border-b border-[var(--border)]">
          <span className="pill text-xs mb-2 block w-fit">Mobile Short</span>
          <h1 className="font-semibold text-sm text-[var(--text)] leading-snug line-clamp-2">{short.topic}</h1>
          {short.domain && (
            <p className="text-[11px] text-[var(--text-faint)] mt-0.5">{short.domain}</p>
          )}
          <p className="text-[11px] text-[var(--text-faint)] mt-1">
            {cards.length} card{cards.length !== 1 ? 's' : ''}
            {short.level && <span className="ml-1.5 pill py-0 px-1.5 text-[10px]">{short.level}</span>}
          </p>
        ...
        {/* Collection & published URL */}
        <div className="px-4 py-3 border-b border-[var(--border)] space-y-2">
          {short.collectionId && (
            <div>
              <p className="text-[10px] text-[var(--text-faint)] uppercase tracking-wide mb-0.5">Collection</p>
              <p className="text-[11px] font-mono text-[var(--text-dim)] break-all">{short.collectionId}</p>
            </div>
          )}
          {short.publishedUrl ? (
            <div>
            ...
            short.status !== 'running' && (
              <div className="space-y-2">
                <button
                  ...
                  className="btn-primary w-full text-xs py-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {publishing ? 'Publishing…' : 'Publish to Educative'}
                </button>
                {publishError && (
                  <p className="text-[11px] text-red-400 leading-relaxed">{publishError}</p>
                )}
              </div>
            )
          )}
        ...
        <div className="flex-1 px-4 py-3">
          {short.status === 'published' && (
            <div className="flex items-center gap-2 text-emerald-600">
              ...
              <span className="text-xs font-medium">Published to Educative</span>
            </div>
          )}
          {short.status === 'failed' && short.errorMessage && (
            <p className="text-xs text-red-400 leading-relaxed">{short.errorMessage}</p>
          )}
          {short.status === 'running' && (
            <p className="text-xs text-amber-600 animate-pulse">Pipeline in progress…</p>
          )}
        </div>
      </aside>
      ...
      <main className="flex-1 min-w-0 py-8 px-6 overflow-x-hidden">
        ...
        <div className="space-y-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="font-semibold text-xl text-[var(--text)] tracking-tight">{short.topic}</h2>
              <p className="text-xs text-[var(--text-faint)] mt-1">
                {cards.length} card{cards.length !== 1 ? 's' : ''} · use ← → to navigate
              </p>
            </div>
            {short.status === 'running' && (
            ...
            {(short.status === 'draft' || short.status === 'failed') && cards.length > 0 && (
              <button
                onClick={publish}
                disabled={publishing}
                className="btn-primary text-xs shrink-0 mt-0.5 disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          {cards.length > 0 ? (
            ...
            <div className="card p-6 text-sm text-[var(--text-faint)]">No cards available.</div>
          )}
        </div>
      </main>
