...
const STAGE_ORDER = [
  ...
  'medium-dna', 'cip-final-pass', 'projects-text-generator', 'projects-reviewer',
...
export default function BlogRunPage() {
  ...
  // Falls back to the whole param for legacy bare-ID links.
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <Link href="/blog" className="text-sm text-[var(--text-dim)] hover:text-white">← New Blog</Link>
          <h1 className="mt-1.5 text-2xl font-bold tracking-tight break-words">
            {final?.title || (running ? 'Generating blog…' : 'Blog generation')}
          </h1>
          {final && (
            <div className="mt-2 flex flex-wrap gap-1.5">
              {final.persona && <span className="pill">{final.persona}</span>}
      ...
      {/* ── Running view: pipeline progress + live draft ── */}
      {running && (
            ...
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  onClick={() => copyContent(activeContentText)}
                >
                  Copy
                </button>
                <button
                  className="btn-secondary text-xs"
                  onClick={() => download(activeContentText, `${final.title || 'blog'}.${activeContentExt}`, activeContentMime)}
                >
                  Download
                </button>
              </div>
